  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 p-5 hover:shadow-md transition-shadow flex flex-col">
      ...
      <div className="flex items-start justify-between mb-3">
        ...
        <span className="px-2.5 py-1 bg-slate-100 text-slate-600 text-[10px] font-black rounded-lg uppercase tracking-wide">
          {course.tag}
        </span>
      </div>
      <h3 className="font-black text-slate-900 text-base leading-snug">{course.title}</h3>
      ...
      <div className="mt-3 mb-4">
        <div className="flex justify-between text-[10px] font-black text-slate-500 mb-1.5">
          <span>Progress</span>
          <span className="text-[#4F39F6]">{course.progress}%</span>
        </div>
        <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
          <div
            className="h-full rounded-full transition-all"
            style={{ width: `${course.progress}%`, background: primary ? '#4F39F6' : '#94a3b8' }}
          />
        </div>
      </div>
      <button
        onClick={() => navigate(`/courses/${course.id}`)}
        className={`mt-auto w-full py-2.5 text-xs font-black rounded-xl transition-colors ${primary
  ...
  return (
    <div className="flex flex-col items-center gap-1.5 flex-1">
      ...
      <div className="w-full flex items-end justify-center" style={{ height: 100 }}>
        <div
          className={`w-7 rounded-t-lg transition-all ${isToday ? 'bg-[#4F39F6]' : 'bg-slate-100'}`}
          style={{ height: `${pct}%`, minHeight: 6 }}
        />
      </div>
      <span className={`text-[10px] font-bold ${isToday ? 'text-[#4F39F6]' : 'text-slate-400'}`}>{day}</span>
    </div>
  );
}
      ...
      <div className="bg-[#6063EE] text-white rounded-[28px] p-8 flex items-center justify-between overflow-hidden relative">
        ...
        <div className="flex gap-4 relative z-10 shrink-0">
          <div className="bg-white/15 backdrop-blur-sm rounded-2xl px-5 py-3.5 text-center min-w-[120px]">
            <p className="text-[9px] font-black text-white/70 uppercase tracking-widest mb-1">Learning Streak</p>
            <p className="text-white font-black text-xl leading-none">🔥 {data?.streak} <span className="text-sm font-black">Days</span></p>
          </div>
          <div className="bg-white/15 backdrop-blur-sm rounded-2xl px-5 py-3.5 text-center min-w-[120px]">
            <p className="text-[9px] font-black text-white/70 uppercase tracking-widest mb-1">Total Points</p>
            <p className="text-white font-black text-xl leading-none">🏆 {data?.totalXP?.toLocaleString()}<span className="text-sm font-black">XP</span></p>
          </div>
        </div>
      </div>
