...
const MOCK_STATS = [
  { label: "Assigned Hackathons", value: 3, icon: Trophy, color: "text-[#4F46E5] bg-indigo-50" },
    ...
    <div className="space-y-6 max-w-5xl">
      ...
      <div className="bg-white border border-slate-100 rounded-[24px] overflow-hidden shadow-sm">
        <div className="px-6 py-4 border-b border-slate-50 flex items-center justify-between">
          <p className="text-sm font-extrabold text-slate-900">Assigned Hackathons</p>
          <button
            ...
            return (
              <div key={h.id} className="px-6 py-4 flex items-center gap-4 hover:bg-slate-50/50 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-extrabold text-slate-900 text-sm truncate">{h.title}</p>
                  <p className="text-xs font-bold text-slate-400 mt-0.5">Deadline: {fmt(h.deadline)}</p>
                  <div className="mt-2 flex items-center gap-2">
                    <div className="flex-1 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full transition-all ${isComplete ? "bg-emerald-400" : "bg-[#4F46E5]"}`}
                ...
                <div className="flex items-center gap-3 shrink-0">
                  ...
                  {!isComplete && (
                    <button
                      onClick={() => navigate("/judge/hackathons")}
                      className="flex items-center gap-1.5 px-3 py-1.5 bg-[#4F46E5] text-white rounded-xl text-xs font-extrabold hover:bg-[#4338CA] transition-all"
                    >
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        >
          ...
          <p className="font-extrabold text-slate-900 text-sm">Start Judging</p>
        ...
        >
          ...
          <p className="font-extrabold text-slate-900 text-sm">View Criteria</p>
