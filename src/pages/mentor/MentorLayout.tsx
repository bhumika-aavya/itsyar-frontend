...
import { ClipboardList, BookOpen, User, LogOut, ChevronDown, GraduationCap, LayoutDashboard, ChevronRight, ChevronLeft } from 'lucide-react';
...
const NAV_ITEMS = [
  { icon: ClipboardList, label: 'Submissions', path: '/mentor' },
...
export default function MentorLayout() {
  ...
  const isActive = (path: string) => {
    if (path === '/mentor')
      return location.pathname === '/mentor' || location.pathname.startsWith('/mentor/submissions');
    ...
    <div className="flex min-h-screen bg-[#F9FAFD] dark:bg-[#111217] transition-colors duration-300">
      ...
      <main className="flex-1 flex flex-col min-w-0">
        <header className="h-16 bg-white dark:bg-[#16171d] border-b border-slate-100 dark:border-[#2e303a] flex items-center justify-between px-8 sticky top-0 z-40">
          <div>
            <p className="text-[10px] font-extrabold text-slate-400 dark:text-slate-500 uppercase tracking-widest leading-none mb-0.5">Mentor</p>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              >
                <div className="w-9 h-9 rounded-full bg-sky-50 dark:bg-sky-950/50 border border-transparent dark:border-[#2e303a] flex items-center justify-center">
                  <span className="text-sky-600 dark:text-sky-400 font-extrabold text-sm uppercase">
                    {user?.fullName?.charAt(0) || 'M'}
                  </span>
                </div>
                <div className="text-right hidden sm:block">
                  <div className="text-sm font-bold text-slate-900 dark:text-slate-100 leading-tight">{user?.fullName || 'Mentor'}</div>
                  <div className="text-[10px] font-extrabold text-sky-600 dark:text-sky-400 uppercase tracking-widest">Mentor</div>
                </div>
