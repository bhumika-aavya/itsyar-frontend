...
import { LayoutDashboard, Scale, ListChecks, User, LogOut, ChevronDown, Award, ChevronRight, ChevronLeft } from 'lucide-react';
...
const NAV_ITEMS = [
  { icon: LayoutDashboard, label: 'Overview', path: '/judge' },
  { icon: Scale, label: 'Judge Hackathons', path: '/judge/hackathons' },
  { icon: ListChecks, label: 'Criteria', path: '/judge/criteria' },
...
export default function JudgeLayout() {
  ...
  const isActive = (path: string) =>
    path === '/judge' ? location.pathname === '/judge' : location.pathname.startsWith(path);
  ...
  return (
    <div className="flex min-h-screen bg-[#F9FAFD] dark:bg-[#111217] transition-colors duration-300">
      ...
      <aside className={`${collapsed ? 'w-[88px] px-4 py-6' : 'w-64 p-6'} bg-white dark:bg-[#16171d] border-r border-slate-100 dark:border-[#2e303a] flex flex-col sticky top-0 h-screen transition-all duration-300 relative z-50`}>
        ...
        <div className={`flex items-center gap-2 mb-2 cursor-pointer ${collapsed ? 'justify-center' : 'px-2'}`} onClick={() => navigate('/judge')}>
        ...
        <div className={`mb-8 ${collapsed ? 'flex justify-center' : 'mx-2'}`}>
          <span className={`inline-flex items-center gap-1.5 py-1 bg-amber-50 rounded-lg ${collapsed ? 'px-2' : 'px-2.5'}`}>
            <Scale size={11} className="text-amber-600 shrink-0" />
            {!collapsed && <span className="text-[11px] font-extrabold text-amber-600 uppercase tracking-widest truncate">Judge Panel</span>}
          </span>
        </div>
      ...
      <main className="flex-1 flex flex-col min-w-0">
        <header className="h-16 bg-white dark:bg-[#16171d] border-b border-slate-100 dark:border-[#2e303a] flex items-center justify-between px-8 sticky top-0 z-40">
          <div>
            <p className="text-[10px] font-extrabold text-slate-400 dark:text-slate-500 uppercase tracking-widest leading-none mb-0.5">Judge</p>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              >
                <div className="w-9 h-9 rounded-full bg-amber-50 dark:bg-amber-950/50 border border-transparent dark:border-[#2e303a] flex items-center justify-center">
                  <span className="text-amber-600 dark:text-amber-400 font-extrabold text-sm uppercase">
                    {user?.fullName?.charAt(0) || 'J'}
                  </span>
                </div>
                <div className="text-right hidden sm:block">
                  <div className="text-sm font-bold text-slate-900 dark:text-slate-100 leading-tight">{user?.fullName || 'Judge'}</div>
                  <div className="text-[10px] font-extrabold text-amber-600 dark:text-amber-400 uppercase tracking-widest">Judge</div>
                </div>
                <ChevronDown size={15} className={`text-slate-400 transition-transform duration-200 ${dropdownOpen ? 'rotate-180' : ''}`} />
              </button>
              {dropdownOpen && (
                <div className="absolute right-0 mt-2 w-52 bg-white dark:bg-[#1f2028] rounded-2xl shadow-2xl border border-slate-100 dark:border-[#2e303a] py-2 z-50">
                  ...
                  <button
                    onClick={() => { navigate('/judge/profile'); setDropdownOpen(false); }}
