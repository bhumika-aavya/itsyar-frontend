...
const fieldCls = (hasErr: boolean) =>
    `w-full h-11 rounded-xl border-2 px-4 bg-slate-50 outline-none transition-all font-medium text-slate-800 text-sm ${hasErr ? 'border-red-300 focus:border-red-400' : 'border-transparent focus:border-[#4F39F6] focus:bg-white'}`;

const textareaCls = (hasErr: boolean) =>
    `w-full rounded-xl border-2 px-4 py-3 bg-slate-50 outline-none transition-all font-medium text-slate-800 text-sm resize-none ${hasErr ? 'border-red-300' : 'border-transparent focus:border-[#4F39F6] focus:bg-white'}`;
...
export default function CreateHackathon() {
    ...
    if (saved) return (
        <div className="max-w-xl mx-auto py-32 text-center space-y-6 px-6">
            ...
            <div>
                <h2 className="text-2xl font-black text-slate-900">{isEdit ? 'Changes Saved' : 'Hackathon Created'}</h2>
            ...
            <form onSubmit={hackHS(onSave)} className="space-y-6">
                ...
                <div className="bg-white border border-slate-100 rounded-3xl p-7 space-y-6">
                    <div className="flex items-center gap-3 pb-2 border-b border-slate-50">
                        <div className="w-9 h-9 bg-indigo-50 rounded-xl flex items-center justify-center">
                            <Trophy size={18} className="text-[#4F39F6]" />
                        </div>
                        <div>
                            <h2 className="font-black text-slate-900">Hackathon Details</h2>
                    ...
                    {/* Title */}
                    <div>
                        <label className="text-xs font-black uppercase tracking-wide text-slate-600 block mb-1.5">Title</label>
                        <input
                            {...hackReg('title')}
                    ...
                    {/* Description */}
                    <div>
                        <label className="text-xs font-black uppercase tracking-wide text-slate-600 block mb-1.5">Description</label>
                        <textarea
                            {...hackReg('description')}
                            rows={4}
                    ...
                    <div className="grid md:grid-cols-3 gap-4">
                        <div>
                            <label className="text-xs font-black uppercase tracking-wide text-slate-600 block mb-1.5">
                        ...
                        <div>
                            <label className="text-xs font-black uppercase tracking-wide text-slate-600 block mb-1.5">Registration Deadline</label>
                ...
                <div className="bg-white border border-slate-100 rounded-3xl p-7 space-y-6">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h2 className="font-black text-slate-900">Problem Statement</h2>
                        ...
                        <button
                            ...
                            className={`flex items-center gap-1.5 px-4 py-2 rounded-xl font-black text-xs transition-all ${includeProblem
                                ? 'bg-red-50 text-red-500 hover:bg-red-100'
                                : 'bg-indigo-50 text-[#4F39F6] hover:bg-indigo-100'}`}
                        >
                        ...
                        <div className="space-y-5">
                            ...
                            <div className="grid md:grid-cols-3 gap-4">
                                <div className="md:col-span-1">
                                    <label className="text-xs font-black uppercase tracking-wide text-slate-600 block mb-1.5">Problem Title</label>
                                    <input
                                        {...probReg('title')}
                                ...
                                <div>
                                    <label className="text-xs font-black uppercase tracking-wide text-slate-600 block mb-1.5">Difficulty</label>
                                    <div className="flex gap-2">
                                        {DIFFICULTY_OPTS.map(d => (
                                            <button
                                                ...
                                                className={`flex-1 py-2.5 rounded-xl font-black text-xs border-2 transition-all ${selectedDifficulty === d
                                ...
                                <div>
                                    <label className="text-xs font-black uppercase tracking-wide text-slate-600 block mb-1.5">Points</label>
                                    <input
                                        {...probReg('points')}
                                        type="number"
                                        min={1}
                            ...
                            {/* Problem Description */}
                            <div>
                                <label className="text-xs font-black uppercase tracking-wide text-slate-600 block mb-1.5">Problem Description</label>
                                <textarea
                                    {...probReg('description')}
                                    rows={6}
                            ...
                            {/* Supported Languages */}
                            <div>
                                <label className="text-xs font-black uppercase tracking-wide text-slate-600 block mb-3">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {SUPPORTED_LANGS.map(lang => (
                                        <button
                                            ...
                                            className={`px-4 py-2 rounded-xl font-black text-xs border-2 transition-all ${selectedLangs.includes(lang)
                                                ? 'border-[#4F39F6] bg-indigo-50 text-[#4F39F6]'
                                                : 'border-slate-100 text-slate-400 hover:border-slate-200'}`}
                                        >
                                            {lang}
                                        </button>
                ...
                <div className="flex items-center justify-end gap-3 pb-8">
                    ...
                    <button
                        ...
                        className="flex items-center gap-2 px-8 py-3 bg-[#4F39F6] text-white rounded-2xl font-black text-sm shadow-xl shadow-indigo-100 hover:bg-[#3f2dd1] disabled:opacity-60 disabled:cursor-not-allowed transition-all active:scale-95"
                    >
                        {saving
